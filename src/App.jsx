...
const MOOD_DESCRIPTIONS = {
  Happy: 'Smiling, laughing, celebrations, good news, warm moments',
  Sad: 'Crying, loss, grief, goodbyes, pain',
  Energetic: 'Battles, danger, urgency, action, sudden escalation',
  Melancholic: 'Nostalgia, distant memories, quiet longing, fading things',
  Calm: 'Peaceful moments, rest, soft dialogue, safe spaces',
  Romantic: 'Confessions, tender moments, longing, embraces, closeness',
  Spooky: 'Dread, haunted places, curses, terror, eerie silence',
  Hopeful: 'Dawn after hardship, believing again, courage, new beginnings',
};
...
function App() {
  ...
  window.electronAPI.saveConfig({ ...config, lastLibrary: currentLibrary });
}, [currentLibrary]);
  ...
  const [moodMix, setMoodMix] = useState(
    Object.fromEntries(MOODS.map(m => [m, true]))
  );
  const moodMixRef = useRef(moodMix);
  useEffect(() => { moodMixRef.current = moodMix; }, [moodMix]);
  ...
  const [playHistory, setPlayHistory] = useState(
    Object.fromEntries(MOODS.map(m => [m, []]))
  );
  ...
  useEffect(() => {
  if (!window.electronAPI?.onTogglePause) return;
  const cleanup = window.electronAPI.onTogglePause(() => audio.togglePause());
  return () => { if (typeof cleanup === 'function') cleanup(); };
}, [audio.togglePause]);
  const loadConfig = async () => {
    if (!window.electronAPI) return;
    const loadedConfig = await window.electronAPI.loadConfig();
    if (loadedConfig.libraries) {
      Object.keys(loadedConfig.libraries).forEach(libName => {
        const lib = loadedConfig.libraries[libName];
        if (!lib.moods) lib.moods = {};

        // MOODS is imported from constants
        MOODS.forEach(m => {
          if (!lib.moods[m]) {
            lib.moods[m] = []; // Initialize missing mood arrays
          }
        });
      });
    }
...
if (loadedConfig.lastLibrary && loadedConfig.libraries[loadedConfig.lastLibrary]) {
  setCurrentLibrary(loadedConfig.lastLibrary);
} else if (libraryNames.length > 0) {
  setCurrentLibrary(libraryNames[0]);
}
  };

  const saveConfig = async (newConfig) => {
   await window.electronAPI.saveConfig({ ...newConfig, lastLibrary: currentLibrary });
    setConfig(newConfig);
  };
  ...
  const loadStarterPlaylist = async () => {
    const newMoods = Object.fromEntries(MOODS.map(m => [m, []]));
    const newTitles = {};
    const newSongs = [];
  ...
  const playFromMix = () => {
    ...
    const activeMoods = Object.keys(moodMixRef.current).filter(m => moodMixRef.current[m]);
    ...
    if (modalType === 'library') {
      ...
      const newConfig = {
        ...
        libraries: {
          ...
          [libraryName]: {
            path: folderPath,
            moods: Object.fromEntries(MOODS.map(m => [m, []])),
            allSongs: musicFiles,
            songTitles: {}
          }
        }
  ...
  const getMoodColor = (mood) => {
    ...
    const themes = {
      ...
      'Romantic': 'radial-gradient(at 0% 0%, #1a1a2e 0%, #6d1b3a 50%, #a0294f 100%)',
'Spooky': 'radial-gradient(at 0% 0%, #0a0a0a 0%, #4a0000 50%, #7f0000 100%)',
      'Hopeful': 'radial-gradient(at 0% 0%, #1a1a2e 0%, #1a3a4a 50%, #87CEEB 100%)',
    };
  ...
  const getMiniMoodStyle = (mood) => {
    const colors = {
      ...
      'Romantic': 'rgba(109, 27, 58, 0.9)',
      'Spooky': 'rgba(97, 12, 12, 0.9)',
      'Hopeful': 'rgba(38, 114, 121, 0.9)',
    };
      ...
      {isMiniMode ? (
        <div className="mini-container" style={{ background: getMiniMoodStyle(currentMood), transition: 'background 2s ease' }}>
          ...
          <div className="mini-mood-buttons">
            {library && Object.keys(library.moods).map(mood => (
            ...
            <div className="mood-container">
              ...
              <div className="mood-grid">
                ...
                {MOODS.map(mood => {
                  ...
                  return (
                    <Tooltip key={mood} content={MOOD_DESCRIPTIONS[mood]}>
                      <button className="mood-card" onClick={() => playRandomFromMood(mood)} style={{ '--mood-color': MOOD_COLORS[mood] }}>
            ...
            <div className="mixer-panel">
              <div className="mixer-header">
                ...
                <div className="mix-visualizer" style={{
                  background: `linear-gradient(90deg, ${Object.keys(moodMix).filter(m => moodMix[m]).length > 0
                    ? Object.keys(moodMix).filter(m => moodMix[m]).map(m => {
                      ...
                      if (m === 'Romantic') return '#FF69B4';
                      if (m === 'Spooky') return '#DC2626';
                      if (m === 'Hopeful') return '#87CEEB';
                      return '#00f2fe'; // Calm
                    }).join(', ')
                    : '#333'
                    })`
